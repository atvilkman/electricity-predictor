...
const en = {
  ...
  vsActualCaption: "Each row is one frozen forecast compared to the price that actually materialised. Multiple rows per target date show the same hour predicted from different lead times.",
  vsActualDate: "Target date",
  vsActualHorizon: "Horizon",
  vsActualPredicted: "Predicted",
  vsActualActual: "Actual",
  vsActualError: "Error",
  accuracyValidationTitle: "Validation accuracy — model vs naive-week baseline",
  accuracyValidationCaption: "Measured on the held-out 60-day validation set (never seen during training). Lower MAE is better; a green ✓ means the model beats simply predicting last week's price.",
  accuracyLiveTitle: "Live accuracy — from real frozen forecasts",
  accuracyLiveCaption: "Computed from actual daily forecast runs vs prices that materialised. Grows as more forecasts mature.",
  accuracyModel: "Model MAE",
  accuracyNaive: "Naive-week baseline",
  accuracyBeatsNaive: "beats naive",
  ...
  aboutSignalsTitle: "Signals tab",
  aboutSignalsP1: "Six indicators that typically influence electricity prices — wind, solar, hydro reservoir levels, nuclear availability, transmission headroom, and demand pressure — each compared to a recent baseline.",
  aboutSignalsP2: "These are informational context for understanding price movements, not inputs the forecast model uses in the same way. The arrow on each tile shows the likely price direction: green/up means downward price pressure, red/down means upward price pressure, yellow/flat means little expected impact.",
  ...
  gridLoadCaption: "Real measured electricity demand across Finland, hourly. Higher demand typically pushes prices up.",
  gridSustainCaption: "Share of electricity produced by renewable, nuclear, and fossil sources, plus the resulting carbon intensity.",
  gridGenCaption: "Hourly breakdown of which energy sources produced Finland's electricity.",
  gridFlowCaption: "Electricity physically moving across borders with Sweden and Estonia — positive values show import, negative-direction pairs show export.",
  tabSignals: "Signals",
  signalsTitle: "What's Driving Prices Right Now",
  signalsIntro: "Six indicators that typically influence electricity prices, compared to their recent baseline. These are informational context, not inputs the forecast model directly uses in the same way.",
  signalWindTitle: "Wind Supply",
  signalWindShows: "Shows: forecast wind speed across Finland + neighboring countries (Sweden, Denmark, Germany, Estonia) for the next 24h, vs the trailing 30-day average.",
  signalWindNotShows: "Does not show: actual wind power output in MW — wind speed is a proxy, not a direct capacity measurement.",
  signalSolarTitle: "Solar Output",
  signalSolarShows: "Shows: forecast solar radiation (Helsinki) for the next 24h, vs the trailing 30-day average.",
  signalSolarNotShows: "Does not show: actual solar panel output in MW — Finland's solar capacity is still small, so this mainly signals cloud cover.",
  signalHydroTitle: "Hydro Reservoir",
  signalHydroShows: "Shows: latest areal snow water equivalent (a proxy for future hydropower availability from snowmelt), vs the same calendar week's average over the last 5 years.",
  signalHydroNotShows: "Does not show: actual reservoir water levels or hydropower output — meaningful mainly in winter/spring; near-zero in summer is normal, not a signal.",
  signalNuclearTitle: "Nuclear Availability",
  signalNuclearShows: "Shows: current nuclear production as % of Finland's total installed nuclear capacity (4,394 MW across 5 reactors).",
  signalNuclearNotShows: "Does not show: scheduled maintenance outages more than a few hours ahead — this is a current snapshot, not a forecast.",
  signalTransmissionTitle: "Transmission Headroom",
  signalTransmissionShows: "Shows: maximum available import/export capacity between Finland and Sweden (SE1, SE3) and Estonia, for tomorrow.",
  signalTransmissionNotShows: "Does not show: actual scheduled flows — this is the ceiling, not what's actually being traded.",
  signalDemandTitle: "Demand Pressure",
  signalDemandShows: "Shows: forecast temperature (Helsinki) for the next 24h, vs the trailing 30-day average — colder or hotter than usual typically raises heating/cooling demand.",
  signalDemandNotShows: "Does not show: actual electricity consumption — temperature is an indirect proxy for demand.",
  signalNoData: "No data available for this signal right now.",
  signalVsBaseline: "vs 30-day avg",
  signalVsSeasonal: "vs 5yr seasonal avg",
  signalOfCapacity: "of installed capacity",
} as const;
...
const fi: Record<TranslationKey, string> = {
  ...
  vsActualCaption: "Jokainen rivi on yksi jäädytetty ennuste verrattuna toteutuneeseen hintaan. Useita rivejä per kohdeaika tarkoittaa, että sama tunti on ennustettu eri läpimenoajoilta.",
  vsActualDate: "Kohdeaika",
  vsActualHorizon: "Horisontti",
  vsActualPredicted: "Ennuste",
  vsActualActual: "Toteuma",
  vsActualError: "Virhe",
  accuracyValidationTitle: "Validointitarkkuus — malli vs. naiivi viikkobaseline",
  accuracyValidationCaption: "Mitattu 60 päivän pidätetyllä validointijoukolla (mallit eivät koskaan nähneet tätä dataa koulutuksessa). Pienempi MAE on parempi; vihreä ✓ tarkoittaa, että malli voittaa edellisen viikon hinnan käyttämisen ennusteena.",
  accuracyLiveTitle: "Live-tarkkuus — oikeista jäädytetyistä ennusteista",
  accuracyLiveCaption: "Laskettu päivittäisistä ennusteajoista verrattuna toteutuneisiin hintoihin. Kasvaa sitä mukaa kun ennusteita kypsyy.",
  accuracyModel: "Mallin MAE",
  accuracyNaive: "Naiivi viikkobaseline",
  accuracyBeatsNaive: "voittaa naiivin",
  ...
  aboutSignalsTitle: "Signaalit-välilehti",
  aboutSignalsP1: "Kuusi indikaattoria, jotka tyypillisesti vaikuttavat sähkön hintaan — tuuli, aurinko, vesivoimavarastot, ydinvoiman saatavuus, siirtokapasiteetti ja kulutuspaine — kukin verrattuna viimeaikaiseen perustasoon.",
  aboutSignalsP2: "Nämä ovat taustatietoa hintaliikkeiden ymmärtämiseksi, eivät ennustemallin käyttämiä syötteitä samalla tavalla. Kunkin kortin nuoli näyttää todennäköisen hintasuunnan: vihreä/ylös tarkoittaa laskevaa hintapainetta, punainen/alas nousevaa hintapainetta, keltainen/vaaka vähäistä odotettua vaikutusta.",
  ...
  gridLoadCaption: "Todellinen mitattu sähkönkulutus Suomessa, tunneittain. Korkeampi kulutus nostaa yleensä hintaa.",
  gridSustainCaption: "Uusiutuvilla, ydinvoimalla ja fossiilisilla lähteillä tuotetun sähkön osuudet sekä niistä syntyvä hiili-intensiteetti.",
  gridGenCaption: "Tuntikohtainen erittely siitä, millä energialähteillä Suomen sähkö on tuotettu.",
  gridFlowCaption: "Sähkön fyysinen siirtyminen rajojen yli Ruotsin ja Viron kanssa — positiiviset arvot kuvaavat tuontia, vastakkaiset suuntaparit vientiä.",
  tabSignals: "Signaalit",
  signalsTitle: "Mikä vaikuttaa hintaan juuri nyt",
  signalsIntro: "Kuusi indikaattoria, jotka tyypillisesti vaikuttavat sähkön hintaan, verrattuna niiden viimeaikaiseen perustasoon. Nämä ovat taustatietoa, eivät suoraan ennustemallin käyttämiä syötteitä samalla tavalla.",
  signalWindTitle: "Tuulivoiman saatavuus",
  signalWindShows: "Näyttää: ennustetun tuulen nopeuden Suomessa ja naapurimaissa (Ruotsi, Tanska, Saksa, Viro) seuraaville 24 tunnille, verrattuna viimeisen 30 päivän keskiarvoon.",
  signalWindNotShows: "Ei näytä: todellista tuulivoiman tuotantoa megawatteina — tuulen nopeus on välillinen mittari, ei suora kapasiteettimittaus.",
  signalSolarTitle: "Aurinkovoiman tuotto",
  signalSolarShows: "Näyttää: ennustetun auringonsäteilyn (Helsinki) seuraaville 24 tunnille, verrattuna viimeisen 30 päivän keskiarvoon.",
  signalSolarNotShows: "Ei näytä: todellista aurinkopaneelien tuotantoa megawatteina — Suomen aurinkokapasiteetti on vielä pieni, joten tämä kertoo lähinnä pilvisyydestä.",
  signalHydroTitle: "Vesivoimavarasto",
  signalHydroShows: "Näyttää: viimeisimmän lumen aluevesiarvon (ennakoi tulevaa lumensulannasta saatavaa vesivoimaa), verrattuna saman kalenteriviikon keskiarvoon viimeisen 5 vuoden ajalta.",
  signalHydroNotShows: "Ei näytä: todellisia allasvedenkorkeuksia tai vesivoiman tuotantoa — merkityksellinen lähinnä talvella/keväällä; lähes nolla kesällä on normaalia, ei signaali.",
  signalNuclearTitle: "Ydinvoiman saatavuus",
  signalNuclearShows: "Näyttää: nykyisen ydinvoimatuotannon prosentteina Suomen koko asennetusta ydinvoimakapasiteetista (4 394 MW, 5 reaktoria).",
  signalNuclearNotShows: "Ei näytä: suunniteltuja huoltoseisokkeja muutamaa tuntia pidemmälle — tämä on nykyhetken tilannekuva, ei ennuste.",
  signalTransmissionTitle: "Siirtokapasiteetti",
  signalTransmissionShows: "Näyttää: suurimman käytettävissä olevan tuonti-/vientikapasiteetin Suomen ja Ruotsin (SE1, SE3) sekä Viron välillä huomiselle.",
  signalTransmissionNotShows: "Ei näytä: todellisia suunniteltuja siirtoja — tämä on yläraja, ei se mitä todellisuudessa siirretään.",
  signalDemandTitle: "Kulutuspaine",
  signalDemandShows: "Näyttää: ennustetun lämpötilan (Helsinki) seuraaville 24 tunnille, verrattuna viimeisen 30 päivän keskiarvoon — tavallista kylmempi tai kuumempi nostaa yleensä lämmitys-/jäähdytystarvetta.",
  signalDemandNotShows: "Ei näytä: todellista sähkönkulutusta — lämpötila on välillinen kulutuksen mittari.",
  signalNoData: "Tälle signaalille ei ole tällä hetkellä dataa.",
  signalVsBaseline: "vs 30pv keskiarvo",
  signalVsSeasonal: "vs 5v kausikeskiarvo",
  signalOfCapacity: "asennetusta kapasiteetista",
};
